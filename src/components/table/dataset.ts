...
export type SortOptions<T extends Record<string, any>> = { by: keyof T, reversed: boolean, with?: SortingFunction<T> };

type GroupingFunction<T> = (nodes: DataSetRowNode<keyof T, T>[]) => DataSetNode<keyof T, T>[];
...
export const createDataSet = <T extends Record<string, any>>(data: T[]): DataSet<T> => {
    ...
    const value = createMemo(() => {
        ...
        if (grouping) {
            value = grouping.with(value as DataSetRowNode<keyof T, T>[]);
        }
